...
//#define C_W_Buff_size	120
#define C_W_Buff_size	125
...
#define TMP_BUF_LEN C_W_Buff_size
	...
	static uint8_t tmp_buf[TMP_BUF_LEN] = {0};
	...
	for (;;) {
		...
		if (xQueueReceive(uart_queue, (void * )&event, (portTickType)portMAX_DELAY)) {
			...
			case UART_DATA:
			{
				...
				//if((event.size != 0)&&(tmp_buf[0] != '\r') && (len != 0))
				if((event.size != 0)&&(len != 0))
				{
					count1++;
					printf("UART_DATA -> C[%d]	RL[%d]\n",count1,len);
					if(xQueueSend( xQueue2, (void *)tmp_buf,0) == pdPASS){//(TickType_t)0)== pdPASS){//(portTickType)portMAX_DELAY)  == pdPASS){//( TickType_t ) 200 )
...
esp_err_t console_uart_init(void)
{
	...
	xTaskCreate(console_uart_task, "uTask", 2048, NULL, 16, NULL);




	/* Create the task for Spp Tx Data, storing the handle. */
	xQueue2 = xQueueCreate( 10, C_W_Buff_size );
	xTaskCreate(
