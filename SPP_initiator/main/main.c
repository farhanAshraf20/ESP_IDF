...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_DISCOVERY_COMP_EVT:
        if (param->disc_comp.status == ESP_SPP_SUCCESS) {
        	//printf("\n-->Far : ESP_SPP_DISCOVERY_COMP_EVT\n\n");
            ESP_LOGI(SPP_TAG, "ESP_SPP_DISCOVERY_COMP_EVT scn_num:%d", param->disc_comp.scn_num);
            for (i = 0; i < param->disc_comp.scn_num; i++) {
            	printf("-- [%d] scn:%d service_name:%s", i, param->disc_comp.scn[i],param->disc_comp.service_name[i]);
    ...
    case ESP_SPP_DATA_IND_EVT:
        //ESP_LOGI(SPP_TAG, "ESP_SPP_DATA_IND_EVT len:%d handle:%d",param->data_ind.len, param->data_ind.handle);
                if (param->data_ind.len < 128) {
                	...
                	snprintf(Buff,(param->data_ind.len - 1),"%s\n", param->data_ind.data);
                	printf("%s",Buff);
                }
        break;
...
static void esp_bt_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_READ_RSSI_DELTA_EVT :
    	printf("Received : ESP_BT_GAP_READ_RSSI_DELTA_EVT\n");
    	break;
